import { For, createSignal, onCleanup, onMount, type Component, type JSX } from "solid-js";
    ...
    <article class={styles.preface}>
      <div ref={bindBody} class={styles.body}>
        ...
        <section id="opening" class={styles.section} data-reveal>
          ...
          <Prose>
            19 年的 GPT3.5，上下文只有可怜的几 K，换到现在的 Agent 上，应该连提示词都塞不进去。用它来生成的游戏只有一个抽象的草图界面；到 26 年的 GPT5.6，百万上下文，已经可以做出具备优秀阴影、音效和物理引擎的赛车游戏。大模型每一年的进步都是指数级别的，从模型本身，到周边的生态变化，都以月为单位在快速迭代。
          </Prose>
        </section>
        ...
        <section id="ecosystem" class={styles.section} data-reveal>
          ...
          <Prose>
            到了甲方这边，变化也很明显。去年，我短暂的体验了一把 OPC，去和工业界、教育界和服务行业的甲方打交道。从 26 年初的小龙虾风潮刮遍全国后，越来越多传统行业也开始尝试了解 AI。理由各不相同，有些想通过 AI 提效，或是增加新的增长曲线，有的则是单纯怕落后于其他竞对，错过新的机会。
          </Prose>
          <Prose>
          ...
          <Prose>
            技术层面上，基模厂商基本上都已经适配到了 100M 上下文，omini 全模态模型开始流行（minimax-h3 已支持全模态输入和输出的能力），能力上思考模式变成标配，Agentic 后训练也成为了每个厂商的技术博客中的常客。包括梁文峰投资人会议上一直在强调的大模型持续记忆能力，可能会成为下一个各大基模厂商押注的点。种种现象都在说明模型基座正在以每半年一个全新高度，甚至更快的速度更新中。
          </Prose>
          <Prose>
        ...
        <section id="project" class={styles.section} data-reveal>
          <h2>WaytoLLM 项目</h2>
          <Prose>
            基于上言，整个大模型的发展可以说日新月异，这个月正在爆火的概念下个月也许就会被推翻，新的观点和架构也在层出不穷。这就造成入门的同学很难从一个角度出发，快速了解整个行业全貌，因为也许这个角度就是错的🤣；另一方面，深入在某个领域的技术或业务同学，也需要不断的更新知识，来适应不断更迭的概念。
          </Prose>
          <Prose>
            此项目正是想从整个生态中找到真正有价值和关键的路径，从而帮助大家串联起整个大模型生态。项目将从模型架构出发，到现有技术框架，项目落地和行业场景，由浅入深地探索“大模型之路”。
          </Prose>
        </section>
        ...
        <section id="closing" class={styles.section} data-reveal>
          ...
          <Prose>
            同时作为开源项目，随时欢迎大家贡献宝贵的项目和学习资料，一起共建！
          </Prose>
